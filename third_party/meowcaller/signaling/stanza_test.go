...
func TestAcceptAndPreacceptShape(t *testing.T) {
	...
	preacceptAction := contentNodes(t, pre)[0]
	preacceptCapability, ok := getChild(t, preacceptAction, "capability")
	if !ok {
		t.Fatal("preaccept capability missing")
	}
	gotCapability, ok := preacceptCapability.Content.([]byte)
	if !ok {
		t.Fatalf("preaccept capability content is %T, want []byte", preacceptCapability.Content)
	}
	if !bytes.Equal(gotCapability, CapabilityPreaccept) {
		t.Errorf("preaccept capability = %x, want %x", gotCapability, CapabilityPreaccept)
	}
	if bytes.Equal(gotCapability, CapabilityOffer) {
		t.Errorf("preaccept capability must differ from offer capability %x", CapabilityOffer)
	}
}
